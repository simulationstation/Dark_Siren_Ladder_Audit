...
from entropy_horizon_recon.dark_sirens_selection import (
    ...
    infer_observing_segment_from_event_name,
    load_o3_injections,
...
from entropy_horizon_recon.gwtc_pe_priors import PeAnalyticDistancePrior, load_gwtc_pe_analytic_priors
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        type=int,
        default=1,
        help="Parallel worker processes for per-event scoring within each run_dir (default: 1). Use 0 for all cores.",
    )
    ap.add_argument(
        "--scale-up",
        action="store_true",
    ...
    ap.add_argument(
        ...
        help=(
            ...
            "  - auto: choose an O3 sensitivity injection file (O3a/O3b/O3) based on the event set and download if missing.\n"
    ...
    if int(args.n_proc) <= 0:
        args.n_proc = int(os.cpu_count() or 1)
    ...
    if hierarchical_pe_mode:
        ...
        if inj_spec.lower() in ("none", "0", "false"):
            ...
            segs = {infer_observing_segment_from_event_name(ev) for ev in cached_events}
            want_auto = inj_spec.lower() == "auto"
            if not want_auto:
                inj_path_try = Path(inj_spec).expanduser().resolve()
                if "o3a" in inj_path_try.name.lower() and ("o3b" in segs):
                    want_auto = True
                    print(
                        f"[dark_sirens] selection: overriding O3a-only injections ({inj_path_try.name}) for an O3b/mixed event set -> auto",
                        flush=True,
                    )
                else:
                    resolved_injections_path = inj_path_try

            if want_auto:
                resolved_injections_path = resolve_o3_sensitivity_injection_file(
    ...
    if inj_spec.lower() in ("none", "0", "false"):
        ...
        # If user provided an explicit O3a-only injections file but the event set contains O3b,
        # silently upgrade to an O3 sensitivity injection release (auto) to avoid a known mismatch.
        segs = {infer_observing_segment_from_event_name(ev) for ev in cached_events}
        want_auto = inj_spec.lower() == "auto"
        if not want_auto:
            inj_path_try = Path(inj_spec).expanduser().resolve()
