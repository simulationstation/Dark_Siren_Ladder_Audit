...
# Strong Planck lensing holdout test (full training + evaluation) that survives disconnects.
#
# Run THIS script under a single nohup:
#
#   out_base="outputs/finalization/planck_lensing_holdout_strong_$(date -u +%Y%m%d_%H%M%SUTC)"
#   mkdir -p "$out_base"
#   nohup bash scripts/launch_planck_lensing_holdout_strong_single_nohup.sh "$out_base" > "$out_base/launcher.log" 2>&1 &
#
# Status:
#   .venv/bin/python scripts/status_out_base.py "$out_base/train_mu"
#   .venv/bin/python scripts/status_out_base.py "$out_base/train_bh"
#
# Stop (kills *currently running* phase):
#   xargs -r kill -TERM < "$out_base/pids_active.txt"
#
# Notes:
...
out_base="${1:-outputs/finalization/planck_lensing_holdout_strong}"
mkdir -p "$out_base"
